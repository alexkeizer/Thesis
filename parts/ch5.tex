...
the very least a QPF\footnote{Informally, we would call $F$ polynomial. But recall that we formalized polynomial functors with a specific structure that might not be preserved by the isomorphism, so formally $F$ might not be considered polynomial.}, this is formalized by \lean{MvQpf.ofPolynomial}.
...
To get rid of the extra variable \lean{ρ}, we take the fixpoint.
\begin{leancode}
...
The composition pipeline is not just an internal detail of the procedure, we also expose it through the \qpf{} macro, whose syntax is inspired by \keyword{def}.
\begin{leancode}
