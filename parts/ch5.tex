...
For the ``head'' type (\leanm{A}), we will take a type that has exactly as many constructors as
...
the shape type is a QPFs (i.e., to provide an instance of \lean{MvQpf}).
...
Namely, by adding an extra type parameter \lean{ρ} and substituting \lean{ρ} for all (co)recursive occurrences of the type
to be defined.
...
\begin{remark}
  ...
  Do note that this does \emph{not} apply when we are adding a new parameter (such as \lean{ρ} in our example) for recursive occurrences in the very first step (to obtain the \lean{Nonrecursive} specification), such variables \emph{do} get substituted in all subexpressions.
  \end{remark}
...
and applications of non-QPFs (such as \lean{α → β} when \lean{α} is a live parameter).
...
Beyond just constructing a (co)datatype, there are a few extra definitions we want to add to make the result more usable. To begin, we want to generate a function for each constructor 
