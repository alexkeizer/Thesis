...
In this case, \lean{σ₁} is supposed to stand for \lean{QpfList ρ}, so we're aiming to define a QPF \lean{Base} which satisfies:
%LEANIGNORE
\begin{leancode}
...
\end{leancode}
Where \lean{σ₁} stands for \lean{QpfList ρ}, as before, and \lean{σ₂} stands for \lean{QpfList (QpfList ρ)}.  
...
Finally, \lean{Base} can simply be defined as the composition of \lean{G} with \lean{![H₁, ..., Hₘ]},
whence the desired type is again obtained by taking the fixpoint.
\begin{leancode}
  def QpfTree α β : TypeFun 2 :=
    MvQpf.Fix (TypeFun.ofCurried Base)
\end{leancode}
...
\begin{enumerate}
  \item Make the specification non-recursive by introducing a new parameter (\cref{sec:rec_types})
  \item Introduce more parameters, to obtain a shape type \lean{Shape} (\cref{sec:comp_pipeline})
  \item Show that \lean{Shape} is a QPF, by (\cref{sec:shape_types})
        \begin{enumerate}
          \item Deriving an equivalent polynomial functor, and
          \item Showing that this polynomial functor is isomorphic to \lean{Shape}
        \end{enumerate}
  \item Use the composition pipeline to solve for the intended values for parameters introduced in step 2 (\cref{sec:comp_pipeline})
  \item Finally, take the fixpoint or cofixpoint (\cref{sec:rec_types})
\end{enumerate}
