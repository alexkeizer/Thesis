...
To that end, we're going to dig deeper into the technical details of the meta-programming system.
We still don't assume knowledge of Lean specifics, but we do require some more familiarity with functional programming (roughly, readers should be familiar with monads, and how they are used for side effects)~\cite{ullrichNotationsHygienicMacro2022,paulinoMetaprogrammingLean}.


\section{Extending Lean's syntax}%
\label{sec:syntax}

Like many modern theorem provers, Lean has facilities that allow us to register custom syntax for specific functions. For example, the following macro allows us to write \lean{Γ ⊢ 0 : Nat} in the familiar syntax for type checking, translating it to \lean{Typing Γ 0 Nat}.
\begin{leancode}
...
We want \data{} to be (mostly) suitable as a drop-in replacement for \inductive{}, so we copy the latter's parser verbatim, replacing the \lean{"inductive"} atom with \lean{"data"}; \codata{} also uses the same syntax, expect the command is now \lean{"codata"}.
\begin{leancode}
...
instead of \lean{builtinCommandParser}.
...
So instead, we have the parser accept exactly the same syntax as \inductive{}, and in the \emph{elaborator} we then throw a custom, informative error when a user tries to use syntax which parses but is unsupported.
In this way, \data{} is truly a drop-in replacement for \inductive{}, syntax-wise, and any errors will guide the user and explain the unsupported syntax.
...
Where \lean{decleration} refers to the parser we defined in the preceding section, which is how Lean
...
At this point, auto-bound implicit variables are added, the constructor type terms are elaborated, and type universes are inferred (if needed), to produce a \lean{InductiveType} object. 
...
Most of this work, obviously besides the steps that add inductive declarations to the environment, is also relevant for \data{} and \codata{} declarations.
...
Since the implementation is intended purely as a prototype, a simpler approach was taken, namely, to 
perform most steps of the procedure on the level of syntax, rather than elaborated expressions. 
The composition pipeline is an exception, for that step we do have to elaborate terms into \lean{Expr}.


Concretely, this means that we defined a \lean{DataView} structure as a thin wrapper around a \data{} or \codata{} syntax tree, just like \lean{InductiveView}.
...
meaning that an identifier \lean{Baz} written directly into a syntax quotation is not actually accessible outside the macro, so that they don't accidentally clash or shadow other identifiers at the call-site. The \lean{mkIdent} function allows us to opt out of hygiene making sure the identifier is just \lean{Baz} as intended.
...
% Recall the overview of the procedure (see \cref{sec:procedure:overview})

% \begin{enumerate}
%   \item\label{enum:impl:make_nonrec}
%     Make the specification non-recursive by introducing a new parameter
%   \item\label{enum:impl:mkShape}
%     Introduce more parameters, to obtain a shape type \lean{Shape}
%   \item\label{enum:impl:mkQpf}
%         Show that \lean{Shape} is a QPF, by 
%           \begin{enumerate}
%             \item Deriving an equivalent polynomial functor, and
%             \item Showing that this polynomial functor is isomorphic to \lean{Shape}
%           \end{enumerate}
%   \item\label{enum:impl:mkBase}
%     Use the composition pipeline to solve for the intended values for parameters introduced in step 2, producing the \lean{Base} functor
%   \item\label{enum:impl:fixpoint}
%     Take the fixpoint (or cofixpoint) of \lean{Base}, getting rid of the parameter introduced in step 1 
%     and producing the desired functor
%   \item\label{enum:impl:aux} 
%     Generate auxiliary constructions
% \end{enumerate}

By design, the steps of the procedure we described in \cref{sec:procedure:overview} closely correspond to the different functions that make up \lean{elabData}s implementation. We will now describe roughly the same overview, but with a focus
...
add the result to the environment (step~\ref{enum:proc:fixpoint}). It will do so both first uncurried form, as \lean{F.Uncurried}, and
