...
With the parser extension in place, the next step is to give our commands their semantics, by defining a corresponding \emph{elaborator}.
...
Besides defining a function of the appropriate type, an elaborator also needs to be decorated with the right attribute.

\begin{leancode}
  @[commandElab declaration] def elabData : Syntax → CommandElabM Unit
\end{leancode}

Where \lean{declaration} is the name of the parser we defined in the preceding section, telling Lean
