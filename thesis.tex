...
Take, for example, the inductive type of lists, as presented in the introduction.
\begin{leancode}
    inductive List α 
    | nil  : List α
    | cons : α → List α → List α
...
Then, define the functor \lean{\ldef{} F$_\lean{nat}$ α := Unit ⊕ α}, where \lean{Unit} is a type with exactly one inhabitant, with the algebra \lean{f : F$_\lean{nat}$ Nat → Nat} that maps the unit case to \lean{zero} and maps every \lean{(n : Nat)} to \lean{succ n}. The inductive properties of \lean{Nat} are exactly what makes this algebra initial.
