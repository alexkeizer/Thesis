...
\begin{todo}
    Insert reference to avigad
\end{todo}
I have implemented a \codata macro in Lean 4 to do exactly this.
For example, the following is automatically compiled into a type that is equivalent to \texttt{InfOrFinList}
...
\begin{todo}
    References
\end{todo}
The work was inspired by an effort to add a (co)datatype package to Isabelle/HOL, 
and greatly based on work by Avigad et al that established how coinductive types can be encoded
(namely, through QPFs, or Quotients of Polynomial Functors) and provided formalizations of the relevant constructions -- albeit in Lean 3, while we are using Lean 4.
...
Chapter \ref{ch:background} will explain what QPFs are;
chapter \ref{ch:porting} will detail the process of porting the QPF formalizations from Lean 3 to Lean 4;
chapter \ref{ch:enhancing} will describe the enhancements we made to thsee formalizations, which go beyond just porting;
chapter \ref{ch:procedure} will establish the translation procedure used by \data and \codata;
chapter \ref{ch:implementing} will go into more technical detail about the (proof of concept)
implementation of these macros, and the Lean 4 meta-programming system;
finally, chapter \ref{ch:conclusion} will show the limitations of the current implementation, discuss potential improvements, and conclude the thesis.

The code for the implementation can be found at \url{https://github.com/alexkeizer/qpf4}
...
The latter is trivial, since \lean{PFin2 0} is uninhabited, and the former we can construct from \texttt{a}.
